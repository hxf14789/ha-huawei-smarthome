"""User-contributed protocol for Huawei product 20GX (领普单键零火墙壁开关 Q3D-HW-W1).

与 20UC（双键 Q3D-HW-W2）/ 153E（三键 Q3D-HW-W3）协议同构，但只有一路按键，故单独成文件
（仓库约定不在适配器之间共享代码）。

证据（2026-09-16 实测）：Profile `.storage/huawei_smarthome/profiles/20GX.json` + 真机状态
（实测 2 台单键面板）。
  switch.on        int RW  唯一一路继电器
  led.brightness   int RW  面板背光/指示灯亮度 0~100（不是灯亮度）
暴露：开关 + 一个 number（指示灯亮度）。
"""
